...
void SignatureMiner(CWallet *pwallet)
{
	...
	while (true) {
     ...
     if (fTryToSync) {
            fTryToSync = false;
            // FIXMEE! get this value from last know checkpoint
            if (vNodes.size() < 40000 || nBestHeight < GetNumBlocksOfPeers())
            {
                MilliSleep(3000);
                continue;
            }
        ...
        if ((GetAdjustedTime() - pindexBest->GetBlockTime()) > ( nTargetSpacing * 2 )) {
	        ...
	        int64_t nFees = -1;
	        auto_ptr<CBlock> pblock(CreateNewBlock( NULL  , &nFees ));        
